...
class SpreadButton: UIView, CAAnimationDelegate {
    ...
    fileprivate static let touchBorderAnimationDuringDefault = 0.2
    ...
    fileprivate var isFinishDrag = true
    ...
    func configureMainButton(_ image: UIImage, highlightImage: UIImage?) {
        ...
        //初始位置
        powerButton.setBackgroundImage(image, for: UIControl.State())
        if let nonNilhighlightImage = highlightImage {
            powerButton.setBackgroundImage(nonNilhighlightImage, for: .highlighted)
        }
        powerButton.addTarget(self, action: #selector(tapPowerButton(_:)), for: UIControl.Event.touchUpInside)
        self.addSubview(powerButton)
    }
    
    func configureCover() {
        cover = UIView(frame: CGRect.init(x: 0, y: 0, width: screenHeight(), height: screenWidth()))
            ...
            case .spreadPositionModeTouchBorder:
                ...
                if location.y < magneticDistance {//上面
                    ...
                    if location.x < self.frame.size.width {
                        destinationLocation = CGPoint(x: self.frame.size.width*2/3, y: self.frame.size.height*2/3)
                    } else if location.x > screenWidth() - self.frame.size.width {
                        destinationLocation = CGPoint(x:screenWidth() - self.frame.size.width*2/3, y: self.frame.size.height*2/3)
                    }
                    ...
                    if location.x < self.frame.size.width {
                        destinationLocation = CGPoint(x: self.frame.size.width*2/3, y: screenHeight() - self.frame.size.height*2/3)
                    } else if location.x > screenWidth() - self.frame.size.width {
                        destinationLocation = CGPoint(x:screenWidth() - self.frame.size.width*2/3, y: screenHeight() - self.frame.size.height*2/3)
                    }
